...
export default function CardDetails({ params }: PageProps) {
  ...
  // Interactive State for Channels Toggle Controls
  const [channels, setChannels] = useState<{ [key: string]: boolean }>({});
  const [spendingControlType, setSpendingControlType] = useState<"allowed" | "blocked">("allowed");
  const [searchQuery, setSearchQuery] = useState("");
  ...
  if (loading) {
    return (
      <div className="w-full h-[60vh] flex flex-col items-center justify-center space-y-4">
        <div className="w-10 h-10 border-[3px] border-[#034EA2] border-t-transparent rounded-full animate-spin" />
        <p className="text-[13px] text-gray-500 font-medium">Loading card details...</p>
      </div>
    );
  }
  ...
  if (error || !card) {
    return (
      <div className="w-full p-12 text-center bg-white border border-gray-100 rounded-xl space-y-4">
        <p className="text-red-500 text-[12px] font-mono">{error || "Could not resolve details configurations."}</p>
        <button onClick={() => router.back()} className="px-4 py-2 bg-[#001533] text-white rounded-lg text-[12px] font-bold">
          Go Back
        </button>
      </div>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            <div className="space-y-4">
              <h3 className="text-[15px] font-bold text-gray-900 border-b border-gray-100 pb-2">Card Details</h3>
              <div className="space-y-3 text-[13px]">
                <div className="flex justify-between"><span className="text-gray-400">Account</span><span className="font-semibold text-gray-900">SUDO / {(card.account?.accountName || "N/A").toUpperCase()}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Business</span><span className="font-semibold text-[#034EA2] underline cursor-pointer">{card.account?.business || "N/A"} →</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Card Holder</span><span className="font-semibold text-gray-900">{card.customer?.name || card.holder}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Currency</span><span className="font-semibold text-gray-990">{card.currency}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Brand</span><span className="font-semibold text-gray-900">{card.brand}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Type</span><span className="font-semibold text-gray-900">{card.type}</span></div>
          ...
          {/* RIGHT SIDE SIDEBAR (3D Card View) */}
          <div className="lg:col-span-5 space-y-6">
            <div className="flex justify-end">
              <button onClick={() => setIsFlipped(!isFlipped)} className="text-[12px] font-semibold text-gray-400 hover:text-[#034EA2] flex items-center gap-1 focus:outline-none">
                🔄 Flip Card
            ...
            <div className="w-full aspect-[1.58/1]" style={{ perspective: "1000px" }}>
              ...
              >
                ...
                <div className="absolute inset-0 rounded-2xl overflow-hidden" style={{ backfaceVisibility: "hidden" }}>
                  <Image src={getCardImage(card.brand || "verve", "front")} alt={`${card.brand} Front`} fill className="object-contain" />
                  <div className="absolute bottom-[28%] left-[7%]">
                    <p className="text-[14px] font-mono tracking-[0.25em] font-medium text-white/90">{card.maskedPan}</p>
                  </div>
                  <div className="absolute bottom-[18%] left-[45%]">
                    <p className="text-[9px] font-mono font-bold text-white/70">
                      {card.expiryMonth && card.expiryYear ? `${card.expiryMonth}/${card.expiryYear}` : ""}
                  ...
                  <div className="absolute bottom-[10%] left-[7%] max-w-[150px]">
                    <p className="text-[10px] font-mono font-bold uppercase tracking-wider text-white/80 truncate">{card.account?.accountName || ""}</p>
                  </div>
                </div>

                {/* BACK SIDE */}
                <div className="absolute inset-0 rounded-2xl overflow-hidden" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
                  <Image src={getCardImage(card.brand || "verve", "back")} alt={`${card.brand} Back`} fill className="object-contain" />
                  <div className="absolute top-[38%] left-[58%] flex items-center gap-3">
                    <span className="text-black font-mono text-[11px] font-bold italic">
                      {(card.cvv || "***").split("").map((char: string, index: number) => <span key={index} className="mx-0.5">{char}</span>)}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-[13px]">
                {txLoading ? (
                  ...
                  filteredTransactions.map((tx: any, i) => (
                    <tr key={tx.id || i} className="hover:bg-gray-50/50">
                      <td className="px-6 py-3.5 font-bold text-gray-800">{tx.merchant?.name || tx.merchant }</td>
                      <td className="px-6 py-3.5 uppercase text-[12px] font-bold text-gray-600">{tx.channel }</td>
                      <td className="px-6 py-3.5 font-mono text-[12px] text-gray-500">{tx.rrn || tx.reference }</td>
                      <td className="px-6 py-3.5 font-bold text-gray-900">{card.currency } {Number(tx.amount || 0).toLocaleString()}</td>
                      <td className="px-6 py-3.5 text-gray-700">{tx.customerName || card.customer?.name }</td>
                      <td className="px-6 py-3.5 font-mono text-[12px] text-gray-600">*{card.last4 }</td>
                    </tr>
                  ))
                ) : (
                  <tr><td colSpan={6} className="py-20 text-center text-gray-400 font-medium">No matching transactions found.</td></tr>
