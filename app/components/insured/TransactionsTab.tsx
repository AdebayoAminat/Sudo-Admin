...
//   };
//
// Add toast notifications for downloads/prints
...
export default function TransactionsTab() {
  ...
  const mapToReceipt = (trx: any) => {
    ...
    return {
      ...
      channel: trx.paid_channel?.toUpperCase(),
      amount: `₦${parseFloat(trx.amount).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`,
      type: isCard ? "CARD PURCHASE" : "WALLET WITHDRAWAL",
      status: trx.status,
      queueStatus: trx.queue_status,
      date: new Date(trx.createdAt).toLocaleString("en-US", {
        month: "short", day: "numeric", year: "numeric",
      ...
      stan: invoice?.stan || "",
      rrn: invoice?.rrn || "",
      bank: trx.user?.sales_point_account?.meta_data?.data?.autoSweepDetails?.bankCode || "",
      accountNo: invoice?.account_number || "",
      narration: trx.narration || "",
    };
  };
  ...
  const handlePrint = () => {
    ...
    const html = `
      ...
      <html>
        <head>
          <title>TRANSACTION_DETAILS_${selectedTrx.trxId}</title>
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .reprint { font-size: 9px; font-weight: 900; letter-spacing: 0.2em; margin-bottom: 16px; }
            .logo-row {
              display: flex;
              align-items: center;
              justify-content: center;
            ...
            .mfb { font-size: 8px; font-weight: 700; letter-spacing: 0.2em; text-transform: uppercase; margin-bottom: 16px; }
            .company { font-size: 12px; font-weight: 900; text-transform: uppercase; line-height: 1.4; margin-bottom: 4px; }
            .address { font-size: 9px; font-weight: 500; text-transform: uppercase; line-height: 1.5; margin-bottom: 20px; }
            .trx-type { font-size: 14px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px; }
            .receipt-no { font-size: 9px; font-weight: 500; margin-bottom: 4px; }
            .dots-top { font-size: 10px; letter-spacing: 0.3em; margin-bottom: 20px; }
            .detail-row {
              display: flex;
            ...
            .detail-row span:first-child { color: #000; white-space: nowrap; }
            .detail-row span:last-child { text-align: right; word-break: break-all; max-width: 180px; }
            .divider { border-top: 1px dashed #000; margin: 16px 0; }
            .dots-mid { font-size: 9px; font-weight: 700; letter-spacing: 0.35em; margin: 8px 0; }
            .amount { font-size: 28px; font-weight: 900; margin: 12px 0 4px; }
            .amount-dots { font-size: 10px; letter-spacing: 0.4em; margin-bottom: 12px; }
            .status { font-size: 14px; font-weight: 900; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 8px; }
            .dots-bottom { font-size: 9px; font-weight: 700; letter-spacing: 0.35em; margin-top: 12px; }
          </style>
        </head>
        <body>
          <div class="receipt">
            <p class="reprint">**** RECEIPT REPRINT ****</p>

            <div class="logo-row">
              <span class="sh-box">SH</span>
              <span class="logo-text">Safehaven</span>
            </div>
            <p class="mfb">Microfinance Bank</p>

            <p class="company">${selectedTrx.company}</p>
            <p class="address">${selectedTrx.address}</p>

            <p class="trx-type">${selectedTrx.type}</p>
            <p class="receipt-no">RECEIPT NO: &nbsp;&nbsp;${selectedTrx.trxId.substring(0, 26)}</p>
            <p class="dots-top">.........................................................................</p>

            ${selectedTrx.channel === "CARD" ? `
              <div class="detail-row"><span>TERMINAL:</span><span>${selectedTrx.terminal}</span></div>
              <div class="detail-row"><span>DATE & TIME:</span><span>${selectedTrx.date}</span></div>
              <div class="detail-row"><span>CARD:</span><span>${selectedTrx.cardType}</span></div>
              <div class="detail-row"><span>CARD EXP:</span><span>${selectedTrx.cardExp}</span></div>
              <div class="detail-row"><span>CLIENT:</span><span>${selectedTrx.client}</span></div>
              <div class="detail-row"><span>PAN:</span><span>${selectedTrx.pan}</span></div>
              <div class="detail-row"><span>AID:</span><span>${selectedTrx.aid}</span></div>
              <div class="detail-row"><span>TRX REF:</span><span>${selectedTrx.trxRef}</span></div>
            ` : `
              <div class="detail-row"><span>DATE & TIME:</span><span>${selectedTrx.date}</span></div>
              <div class="detail-row"><span>BANK:</span><span>${selectedTrx.bank}</span></div>
              <div class="detail-row"><span>ACCOUNT NO:</span><span>${selectedTrx.accountNo}</span></div>
              <div class="detail-row"><span>NARRATION:</span><span>${selectedTrx.narration}</span></div>
              <div class="detail-row"><span>TRX REF:</span><span>${selectedTrx.trxRef}</span></div>
            `}

            <div class="divider"></div>

            <p class="amount">${selectedTrx.amount}</p>
            <p class="amount-dots">........................</p>

            <p class="status">${selectedTrx.status === "Success" ? "SUCCESS" : "FAILED"}</p>

            ${selectedTrx.status === "Failed" ? `
              <div class="divider"></div>
              <div class="detail-row"><span>RESPONSE CODE:</span><span>${selectedTrx.responseCode}</span></div>
              <div class="detail-row"><span>MESSAGE:</span><span>${selectedTrx.message}</span></div>
              <div class="detail-row"><span>STAN:</span><span>${selectedTrx.stan}</span></div>
              <div class="detail-row"><span>RRN:</span><span>${selectedTrx.rrn}</span></div>
            ` : ""}

            <p class="dots-bottom">******** ******</p>
          </div>
        </body>
      </html>
    `;
    ...
    link.href = url;
    link.download = `TRANSACTION_DETAILS_${selectedTrx.trxId}.html`;
    try {
      link.click();
      ...
      {selectedTrx && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-xl shadow-2xl max-w-[340px] w-full overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="pb-5 px-5 bg-white flex items-center justify-center">
              <button
                onClick={handlePrint}
                className="flex items-center justify-center gap-2 text-gray-800 text-[12.5px] font-bold hover:opacity-60 transition-all outline-none bg-transparent border-none cursor-pointer"
              >
