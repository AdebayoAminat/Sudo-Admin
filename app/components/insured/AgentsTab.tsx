...
export default function AgentsTab() {
  ...
  // Fetch locations when agent is selected
  useEffect(() => {
    if (!selectedAgentId || activeSubTab !== "locations") return;
    ...
    const fetchLocations = async () => {
      ...
      try {
        ...
        const res = await insuredService.getLocations(membershipId, currentPage, pageSize) as any;
        if (res?.data) {
      ...
      <div className="space-y-6 animate-in fade-in duration-200">
        <div className="flex justify-between items-center">
          <p className="text-[13px] text-gray-400">
            ...
            of{" "}
            <span className="text-[#1D2939] font-bold">{totalCount}</span> Agents
          </p>
