...
// // CRITICAL: Add 'export' and move this OUTSIDE the component
// export const physicalCardsMock = [
//   {
//     id: "1",
//     holder: "Platnova Technologies Limited",
//     last4: "2661",
//     brand: "verve",
//     type: "Physical",
//     status: "Active",
...
//   {
//     id: "2",
//     holder: "SAW TECHNOLOGIES LIMITED",
//     last4: "6177",
//     brand: "verve",
//     type: "Physical",
//     status: "Active",
...
//           Showing 0 - {physicalCardsMock.length} of 1,760 Physical Cards
//         </span>
...
// export default function PhysicalCardsList({ onSelect }: ListProps) {
//   // Instantiate the service using useMemo to prevent re-creation on every render
...
//     async function fetchPhysicalCards() {
//       setLoading(true);
//       setError(null);
...
//         // Client-side isolation: filter out only physical cards safely without altering case
//         const physicalCards = cardsArray.filter(
//           (card: any) => card.type && card.type.toString().toLowerCase() === "physical"
//         );
//         setCards(physicalCards);
//       } else {
//         // Fallback error messaging checking for backend permission failure structures
//         const errorMsg = response?.error?.message || response?.error || "Failed to load physical cards.";
//         setError(errorMsg);
//       }
//       setLoading(false);
//     }
...
//   // Handle client side search/filter matching accountName, maskedPan, status, or brand
...
//   // 2. Error State (Handles Backend/Permission Restrictions safely)
//   if (error) {
//     return (
...
//   return (
//     <div className="space-y-6">
//       {/* Search and Metadata Controller Row */}
//       <div className="flex justify-between items-center">
//         <span className="text-[12px] text-gray-400 font-medium">
//           Showing {filteredCards.length} Physical Card{filteredCards.length !== 1 ? 's' : ''}
//         </span>
...
//             <input
//               type="text"
//               placeholder="Search by holder, pan, status..."
//               value={searchTerm}
...
//           <p className="text-[13px] text-gray-400">No physical cards located.</p>
//         </div>
//       ) : (
//         /* 4. Populated Grid Layout */
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
...
//             // Format expiry layout as MM/YYYY or MM/YY
...
//       const response = await cardsService.getAllCards(currentPage, itemsPerPage, "physical");
...
//           {itemRangeText} of {totalRecords.toLocaleString()} Physical Cards
//         </span>
...
//       {filteredCards.length === 0 ? (
//         <div className="p-12 text-center border border-dashed border-gray-200 rounded-xl bg-gray-50/30">
//           <p className="text-[13px] text-gray-400">No physical cards located on this page view sheet.</p>
//         </div>
//       ) : (
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
//           {filteredCards.map((card) => {
...
//                   ...card,
//                   id: cardId,
//                   type: displayType,     // Passes "Physical" instead of "physical"
...
export default function PhysicalCardsList({ onSelect, type }: ListProps) {
  ...
  useEffect(() => {
    ...
    async function fetchPhysicalCards() {
      ...
      const response = await cardsService.getAllCardsForACustomer(
        currentPage,
        itemsPerPage,
        "physical",
        debouncedSearch || ''
  ...
  }, [cardsService, currentPage, debouncedSearch]);
  ...
  // No more client-side search filter — API handles it
  const paginationRange = useMemo(() => {
    const range: number[] = [];
    let start = Math.max(0, currentPage - 2);
    let end = Math.min(totalPages - 1, start + 4);
      ...
      {cards.length === 0 ? (
        <div className="p-12 text-center border border-dashed border-gray-200 rounded-xl bg-gray-50/30">
          <p className="text-[13px] text-gray-400">
            {debouncedSearch
              ? `No physical cards found matching "${debouncedSearch}".`
              : "No physical cards located on this page view sheet."}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
