...
//       // 1. Explicitly filter out anything that isn't an account type item
//       if (acc.type !== "account") return false;

//       // 2. Currency filter check
//       const matchesCurrency =
//         currencyCode === "All" || acc.currency === currencyCode;

//       // 3. Search parameter matching text fields
//       const searchLower = search.toLowerCase();
//       const title = acc.accountName || "";
...
//     });
//   }, [allAccounts, currencyCode, search]);
...
//   const startRange = filteredAccounts.length === 0 ? 0 : (currentPage - 1) * pageSize + 1;
//   const endRange = Math.min(currentPage * pageSize, filteredAccounts.length);
...
//         <div>
//           <h2 className="font-bold text-[#1D2939] text-sm mb-2">Accounts</h2>
//           <span className="text-[12px] text-gray-400 block">
//             Showing {startRange} - {endRange} of {filteredAccounts.length} Accounts
//           </span>
//         </div>
...
//           >
//             <Image src="/images/chevron-left.svg" alt="prev" width={18} height={18} />
//           </button>
...
//                       : "text-gray-500 hover:bg-gray-50 hover:text-[#034EA2]"
//                   }`}
//                 >
//                   {pageNum}
//                 </button>
...
//             className="p-2 rounded-xl border border-gray-100 disabled:opacity-30 hover:bg-gray-50 transition-all active:scale-95"
//           >
//             <Image src="/images/chevron-left.svg" alt="next" width={18} height={18} className="rotate-180" />
//           </button>
//         </div>
//       )}
//     </div>
...
//   // Server-side pagination tracking (API starts at 0 index)
//   const [currentPage, setCurrentPage] = useState(0); 
//   const [totalPages, setTotalPages] = useState(1);
//   const [totalItems, setTotalItems] = useState(0);
  
//   const pageSize = 25; // Maintained at 25 per page
//   const accountsService = useMemo(() => new AccountsService(), []);
...
//       setLoading(true);
//       try {
//         // Pass "account" type filter to the backend request
//         const res = await accountsService.getAllAccounts(currentPage, pageSize, "account");
...
//   const filteredAccounts = useMemo(() => {
//     return allAccounts.filter((acc) => {
//       // Safety client-side check fallback
//       if (acc.type !== "account") return false;
...
//           <h2 className="font-bold text-[#1D2939] text-sm mb-2">Accounts</h2>
//           <span className="text-[12px] text-gray-400 block">
//             Showing {startRange} - {endRange} of {totalItems} Accounts
//           </span>
//         </div>
...
export default function AccountsList({
  ...
  // Calculates pagination range for selection blocks matching your requested flow
