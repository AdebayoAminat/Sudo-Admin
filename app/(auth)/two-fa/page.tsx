...
//         setLoading(false);
//         toast.success( <div>
//     <p className="font-semibold">Two factor Authentication successful.</p>
//     <p className="text-xs text-gray-500">User login successful</p>
//   </div>);
//         router.push("/dashboard");
//       } else {
//         setLoading(false);
...
export default function TwoFAPage() {
  ...
  const [errorMessage, setErrorMessage] = useState("");
  const [loading, setLoading] = useState(false);
  const [hasError, setHasError] = useState(false); // Track if current PIN attempt failed
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]);

  const handlePinChange = (value: string, index: number) => {
    if (!/^\d*$/.test(value)) return;
    
    // Clear error flag as soon as user starts typing again
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (statusCode === 200 || statusCode === 202) {
        ...
        setLoading(false);
        setHasError(true);
        setPin(["", "", "", ""]); // Reset inputs so auto-trigger stops firing
        inputRefs.current[0]?.focus(); // Focus first box for easy re-entry

        const msg = request?.data?.message || "Incorrect PIN.";
        setErrorMessage(msg);
        toast.error(msg);
      }
    } catch (error: any) {
      ...
      const msg = error?.response?.data?.message || "Incorrect PIN.";
      setErrorMessage(msg);
      toast.error(msg);
    }
  };

  // Only auto-trigger verification if all 4 digits are entered AND there isn't an active error state
  useEffect(() => {
    if (
      pin.length === 4 &&
      pin.every((digit) => /^\d$/.test(digit)) &&
      !loading &&
      !hasError &&
      step === "pin"
    ) {
